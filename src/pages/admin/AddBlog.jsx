...
const onSubmitHandler = async (e) =>{
  ...
  // ✅ Prevent duplicate "Blog added successfully" from incorrect error toast
  if (errMsg !== "Blog added successfully") {
    toast.error(errMsg);
  }
}finally{
    setIsAdding(false)
     ...
     <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
          ...
          {loading && (
            <div className='absolute right-0 top-0 left-0 bottom-0 flex items-center justify-center'>
              <div className='w-8 h-8 rounded-full border-2 border-t-white animate-spin'>
               ...
               <button disabled={loading} onClick={generateContent} className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer' type='button'>Generate with AI</button>
        </div>
        <p className='mt-4'>Blog Category</p>         
          ...
          <button disabled={isAdding} className='mt-8 w-40 h-10 bg-primary text-white rounded cursor-pointer text-sm' type='submit'>{
            isAdding?
            'Adding...':'Add Blog'
            }</button>
      </div>
